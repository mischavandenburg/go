...
// shuffles the deck
func (d deck) shuffle() {
	// Before we used a pseudo random generator with rand.Intn, now we are going to create a proper one
	// we start with creating a seed based on a Unix nanosecond timestamp generated by UnixNano
	source := rand.NewSource(time.Now().UnixNano())
	// this creates a random number from the previously generated seed which we stored in "source". It has type Rand
	r := rand.New(source)

	// iterate over the deck of cards
	for i := range d {
		// r is of type Rand and this type also has an Intn function
